...
export const useIELTSCourse = (useMockByDefault = import.meta.env.DEV) => {
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);
  const [useMockData, setUseMockData] = useState(useMockByDefault);

  useEffect(() => {
    const fetchData = async () => {
      // If using mock by default, skip API call
      if (useMockByDefault) {
        setData(mockData);
        setUseMockData(true);
        setLoading(false);
      ...
      try {
        ...
        if (import.meta.env.DEV) {
          console.log('Using mock data due to API unavailability:', err.message);
        }
